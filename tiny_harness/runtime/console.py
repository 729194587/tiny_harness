...
class ConsoleEventLogger:
        ...
        try:
            ...
            if self._is_live() and not self.verbose:
                if event_type == EventType.MODEL_REQUESTED:
                    self._write(f"正在请求模型 · 轮次 {short(d.get('turn', '?'))} · 工具 {self._tool_calls} · 上下文 {_human_number(d.get('context_tokens'))}", style=_DIM_GRAY)
                elif event_type == EventType.TOOL_STARTED:
                    self._write(f"正在调用工具 · {short(d.get('tool_name', 'tool'))} · 工具 {self._tool_calls}", style=_DIM_CYAN)
        finally:
            self._persistent = False
            if root_end:
                self.clear_live()
    ...
    def _emit(self, event_type: EventType, data: Mapping[str, Any] | None = None) -> None:
        ...
        if event_type == EventType.TOOL_CALLED:
            self._tool_calls += 1
            self._calls[key] = {k: short(d[k]) for k in ("tool_name", "name") if k in d}
        if self.verbose:
    ...
    def _update(self, event_type: EventType, d: Mapping[str, Any], key: tuple[Any, ...],
        ...
        tool = short(d.get("tool_name", "")) or short(metadata.get("tool_name", "")) or "tool"
        if event_type == EventType.TOOL_RESULT:
            ...
            if aggregate and d.get("outcome") == "returned":
                ...
                self._flush_tool_burst()
                self._write(f"✗ {tool} failed: {short(d.get('error_type', 'Error'))}", style=_RED, error=True)
            ...
            if aggregate:
                ...
                reason = {EventType.TOOL_DENIED: "permission denied",
                          ...
                          EventType.TOOL_HOOK_FAILED: f"hook failed: {short(d.get('error_type', 'Error'))}"}[event_type]
            ...
            if aggregate:
                self._write(f"✗ Subagent failed: {short(d.get('error_type', 'Error'))}", style=_RED, error=True)
            ...
            if aggregate:
                ...
                detail = short(d.get("error_type", d.get("error_kind", "")))
    ...
    def summary(self, *, model: str, stream: TextIO | None = None) -> str:
        ...
        text = "\n".join(("────────────────", "", "Run summary", f"model: {short(model)}",
