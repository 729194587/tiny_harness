...
class ContextCompactor(ContextArtifacts):
    ...
    SUMMARY_SYSTEM = (
        "Summarize only factual task information from the supplied history. "
        "Do not judge readiness, completion, whether unknowns block progress, or "
        "whether a root cause is confirmed. Do not decide whether to begin "
        "implementation or answer the user. Do not recommend next actions, create "
        "plans or TODOs, or give progress guidance. Apply these restrictions to prior "
        "checkpoints too; omit their decision fields and instructions. "
        ...
        "Use the following order, without inventing facts to fill a section.\n"
        "1. Task objective and constraints: the original request and important constraints.\n"
        "2. Observations and evidence: observed behavior and reproduction results; "
        "relevant inspected files, functions and code paths; supporting traces, "
        "commands and git history.\n"
        "3. Code modifications: changes actually made and relevant file/symbol locations, "
        "including explicitly reported absence of changes.\n"
        "4. Verification results: tests and checks actually executed, results and scope, "
        "including failures and explicitly untested cases.\n"
        "5. Unresolved factual questions: distinguish facts from hypotheses. "
        "Preserve contradictory evidence and uncertainty; do not "
        ...
        "confirmed or proven; retain its limited verification scope. Describe what "
        "remains unknown without classifying it as blocking or prescribing investigation.\n"
        ...
        "redundant evidence and information retained merely for completeness. "
        "Integrate any prior checkpoint into one factual snapshot without "
    ...
    def _summary_request(
        ...
        *,
        ...
        while input_limit >= 0:
            ...
            if context_token_count(request, self.tools, self.token_meter) <= limit:
                return request
            if input_limit == 0:
                break
        ...
        *,
        ...
        self.event_logger.emit(
            EventType.CONTEXT_SUMMARY_REQUESTED,
            {
                "reason": "reactive",
                "input_tokens": context_token_count(
                    summary_request, self.tools, self.token_meter
                ),
            },
        )
        response = self._summary_complete(summary_request, self.tools)
        self.event_logger.emit(
            EventType.CONTEXT_SUMMARY_RESPONDED,
            {
                "reason": "reactive",
